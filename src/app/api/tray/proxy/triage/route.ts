 ...
 * Proxies triage requests to Groq (JSON mode).
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Proxy to Groq with JSON mode (non-streaming for triage)
    ...
    // Record usage
    await recordUsage(auth.deviceId, auth.userId, auth.tier, 'triage')
