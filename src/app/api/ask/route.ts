...
export async function POST(req: NextRequest) {
  try {
    ...
    // By now the LLM call has had ~100-200ms to run alongside the SQL queries above
    ...
    if (expandedPhrases.length > 0) {
      const expandedKeywords = Array.from(new Set(
        expandedPhrases.flatMap(phrase => extractKeywords(phrase))
          .filter(kw => !keywords.includes(kw)) // only truly new terms
      ))
    ...
    const scored = allRecords.map(r => {
      ...
      // Temporal date range boost — if query is about a specific period,
      // strongly boost records that fall in that window
      if (dateRange) {
        ...
        if (recordDate >= dateRange.start && recordDate <= dateRange.end) {
          ...
          // Penalise out-of-range records for temporal queries so they don't crowd out results
    ...
    // Await intent classification before we need it for scoring + extraction
    ...
    // ── Multi-hop reasoning ──────────────────────────────────────────────────
    // By this point the detection LLM call has had ~300-500ms to run alongside
    // retrieval — for most queries it will already be resolved.
    let multiHopContext = ''
    try {
      const multiHopPlan = await multiHopPromise
      if (multiHopPlan) {
        // Hop 1: extract the bridge fact from current top memories
...
Answer (or "not found" if genuinely absent):`
        ...
        if (hop1Answer && !hop1Answer.toLowerCase().includes('not found')) {
          // Hop 2: use the bridge fact to retrieve additional records
          ...
          if (hop2Terms.length > 0) {
            ...
            // Merge Hop 2 records into top (avoid duplicates, cap total)
...
From each memory below, extract ALL numbers, amounts, counts, durations, or financial figures relevant to this question.
For each number: note the value, unit/currency, what it refers to, and the date if known.
Output JSON: {"numbers": [{"value": 5000, "unit": "$", "description": "marketing spend Q1", "date": "2026-01", "memory": 1}]}
Skip memories with no relevant numbers. Be precise — quote exact figures.
...
JSON output only:` : null
    ...
    // Conflict detection: find contradictions across memories (for complex multi-memory queries)
...
Output JSON: {"conflicts": [{"description": "brief description of the conflict", "memories": [1, 3], "type": "date|status|assignment|number|decision"}]}
Return {"conflicts": []} if no clear contradictions exist. Only flag REAL conflicts, not just updates or new information.
...
JSON output only:` : null
    ...
    const [profilesResult, extractionResult, numberResult, conflictResult] = await Promise.all([
      ...
      // Fact extraction pass (complex intents, not aggregation — aggregation has its own pass)
      ...
      // Number extraction pass (aggregation intent only)
      ...
      // Conflict detection pass (multi-memory intents only)
    ...
    // Conflict context: surfaced contradictions get injected before memories
    ...
    // For aggregation: inject extracted numbers as structured context
    ...
    const prompt = `You are Reattend — the user's personal AI memory assistant and thinking partner. You have access to everything they have saved.

CRITICAL RULES:
- Start your answer directly — no preamble like "Based on your memories" or "From your notes"
...
ANSWER:`
    ...
    const followUpInstruction = '\n\n---FOLLOWUPS---\nWrite exactly 3 follow-up questions, each grounded in specific details from these memories — genuinely useful for this user, not generic. One per line, starting with "- ".'

    const stream = await llm.generateTextStream(prompt + followUpInstruction)
