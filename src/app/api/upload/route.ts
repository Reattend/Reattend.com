  ...
  try {
    ...
    if (textForAI.trim().length > 10) {
      ;(async () => {
        try {
          const llm = getLLM()
          const prompt = PROMPTS.triage(textForAI)
          const result = await llm.generateJSON(prompt, triageResultSchema)

          const tags = Array.from(new Set([...result.tags, ...fileTags]))
          ...
          for (const entity of result.entities) {
            ...
            if (!existing) {
              ...
              await db.insert(schema.entities).values({
                id: entityId, workspaceId, kind: entity.kind as any, name: entity.name, normalized,
              })
              existing = { id: entityId } as any
            }
            await db.insert(schema.recordEntities).values({ recordId, entityId: existing!.id })
          }
